...
class Saving extends CI_Controller {
	...
	public function investigations($InvestigationName,$InvestigationFee,$InvestigationAvailability,$InvestigationSummary){
		$tableName = "investigation_information";
		$id=$this->new_id('investigation_id',$tableName);
		$data = array(
		'investigation_id' => $id ,
	...
	public function doctor($DoctorFirstName,$DoctorLastName,$DoctorMobileNo,$DoctorPhoneNo,$DoctorAddress,$DoctorCity,$DoctorPinCode,$DoctorState,$DoctorSpecialist,$DoctorComments,$ConsultationCharges){
		...
		$this->saveDataToDb($tableName, $data);
		$this->addConsultationFee($id, $ConsultationCharges);

	}

	public function addConsultationFee($id, $ConsultationCharges){

		$tableName = "consulting_info";

		$data = array(
		'doctor_id' => $id ,
		'consulting_charge' => $ConsultationCharges
		);
	...
	function saveDataToDb($tableName, $data){
		$status = $this->db->insert($tableName, $data); 
		if($status){
			return "Success";
		}else{
	...
	public function visitor_record($patentId, $doctorId, $patentTitle, $patentFirstName, $patentLastName, $patentAge, $patentGender, $patentMartialStatus,$ReferenceName, $ReferenceDiscount, $patentAddress, $patentCity, $patentState, $patentPinCode, $patentCountry, $patentPhoneNo, $patentMobileNo, $investigationId){
		...
		if($patentId=='' || $patentId==' ' || $patentId==null){
			...
			'registereddated'=>$this->currentDate()
		);
